...
Route::get('/goal-invites/{token}', function (string $token) use ($name) {
    ...
    return view('invite', ['kind' => 'goal', 'name' => $member->goal?->name ?? 'Shared goal', 'status' => $member->status, 'email' => $member->email, 'inviterName' => $name($member->inviter ?? $member->goal?->owner), 'appLink' => 'salarytracker://goal-invites/'.$member->token]);
...
Route::get('/wallet-invites/{token}', function (string $token) use ($name) {
    ...
    return view('invite', ['kind' => 'wallet', 'name' => $member->wallet?->name ?? 'Shared account', 'status' => $member->status, 'email' => $member->email, 'inviterName' => $name($member->inviter ?? $member->wallet?->owner), 'appLink' => 'salarytracker://wallet-invites/'.$member->token]);
})->where('token', '[A-Za-z0-9]+');

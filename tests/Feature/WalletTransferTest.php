...
class WalletTransferTest extends TestCase
{
    ...
    public function test_transfers_move_money_between_wallets(): void
    {
        ...
        // ₱500 cash-in to GCash with a ₱15 fee: cash loses 515, GCash gains 500.
        $transfer = $this->postJson('/api/wallet-transfers', ['from_wallet_id' => $cash['id'], 'to_wallet_id' => $gcash['id'], 'amount' => 500, 'fee' => 15, 'transfer_date' => '2026-09-22', 'notes' => 'Cash-in'])
            ->assertCreated()
            ->assertJsonPath('data.from_wallet.name', 'Cash')
            ->assertJsonPath('data.to_wallet.name', 'GCash')
            ->json('data');
        $this->assertSame(15.0, (float) $transfer['fee']);
        ...
        $this->assertSame(2000.0 - 515, (float) $cashNow['balance']);
        $this->assertSame(515.0, (float) $cashNow['transfers_out']);
        ...
        $this->assertSame(15.0, (float) $list['fees']);
        ...
        $graphql = $this->postJson('/api/graphql', ['query' => '{ walletTransfers(range: "period") { total fees transfers { id amount fee from_wallet { name } to_wallet { name } } } wallets { name balance transfers_in transfers_out } }'])
            ->assertOk();
        ...
        $this->assertSame(1485.0, (float) collect($graphql->json('data.wallets'))->firstWhere('name', 'Cash')['balance']);
        ...
        $this->putJson("/api/wallet-transfers/{$transfer['id']}", ['amount' => 300, 'fee' => null])->assertOk()->assertJsonPath('data.fee', 0);
        $wallets = collect($this->getJson('/api/wallets')->assertOk()->json('data'));
